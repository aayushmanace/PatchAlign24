...
from torch.utils.tensorboard import SummaryWriter
# get model
from Models.got_losses_multitask_ddi_fitz import Network, Confusion_Loss, Supervised_Contrastive_Loss
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            for n_iter, batch in enumerate(dataloaders[phase]):
                ...
                label_partition = batch["partition"]
                # print("label_c: ", label_c, "\nlabel_t: ", label_t)
                # NOISE INJECTION
                # if phase == 'train':
                #     num_samples_to_change = int(noise_ratio* label_c.shape[0])
                #     selected_indices = np.random.choice(label_c.shape[0], num_samples_to_change, replace=False)
                #     for idx in selected_indices:
                #         random_label = np.random.choice([0,1])
                #         label_c[idx]=random_label
                  
                label_c, label_t = torch.from_numpy(np.asarray(label_c)).to(device), torch.from_numpy(np.asarray(label_t)).to(device)
                label_partition= torch.from_numpy(np.asarray(label_partition)).to(device)
                optimizer.zero_grad()
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    # print(output[4])
                    predicted = torch.max(output[4], 1)
                    # print(predicted, '=================', label_partition)
                    multitask_loss = criterion[4](output[4],label_partition)

                    # print("multitask_loss: ", multitask_loss, " loss0: ", loss0, " loss1: ", alpha*loss1, " loss2: ", loss2, " loss3: ", loss3*beta )
                    loss = loss0 + loss1*alpha + loss2 + loss3*beta + gamma_m*multitask_loss
...
class SkinDataset():
    ...
    def __getitem__(self, idx):
        ...
        if self.dataset_name == 'fitzpatrick':
            mid = self.df.loc[self.df.index[idx], 'mid'] 
            partition = self.df.loc[self.df.index[idx], 'class_indices'] 
        else:
            mid = 0
            partition = self.df.loc[self.df.index[idx], 'class_indices']
...
def custom_load(
        ...
        image_dir='fitzpatrick17k/data/finalfitz17k',
        ):
    if dataset_name == 'ddi':
        image_dir = 'images'
...
if __name__ == '__main__':
    ...
    random_state_var_1=random_seed
    random_state_var=random_seed

    torch.manual_seed(random_state_var_1)
    random.seed(random_state_var_1)
    np.random.seed(random_state_var_1)
    ...
    if dev_mode == "dev":
        if dataset_name == 'ddi':
            df = pd.read_csv('ddi_with_label_indices.csv').sample(300)
        else:
            df = pd.read_csv("fitzpatrick17k_known_code_with_label_indices.csv").sample(1000)
    else:
        if dataset_name == 'ddi':
            df = pd.read_csv('ddi_with_label_indices.csv')
        else:
            df = pd.read_csv("fitzpatrick17k_known_code_with_label_indices.csv")
    ...
    for holdout_set in ["random_holdout"]: # ["expert_select","random_holdout", "a12", "a34","a56", "dermaamin","br"]:
        ...
        for indexer, label in enumerate([level]):
            ...
            model_ft = Network('vit_disco', [len(label_codes), 6], pretrained=True,num_multitask_classes=114)
            ...
            nn.CrossEntropyLoss(), Supervised_Contrastive_Loss(0.1, device),nn.CrossEntropyLoss()]
            ...
            print("gold")
            training_results.to_csv("training_{}_{}_{}_{}_btachsize{}.csv".format(model_name, n_epochs, label, holdout_set,batch_size))
            ...
            if label == "low":
                ...
                df_x=pd.DataFrame({
                                    ...
                                    "prediction_probability": flatten(p_list),
                                    "prediction": flatten(prediction_list)})
            df_x.to_csv("results_{}_dataset{}_epochs{}_random_seed1{}_batch_size{}_multitask.csv".format(model_name,dataset_name, n_epochs, random_state_var_1, batch_size),
                            index=False)
