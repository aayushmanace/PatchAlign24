...
def IPOT_distance_torch_batch_uniform(C, M, bs, n, m, iteration=50):
	C = C.float().cuda()
...
def IPOT_torch_batch_uniform(C, M, bs, n, m, beta=0.5, iteration=50):
	...
	#else:
	#T = 0.5*(torch.ones(bs, n, m).cuda()+M.cuda())
	#T_ = M.clone().cuda()
	T  = M.cuda()#torch.round(F.normalize(torch.abs(torch.zeros_like(M)))).cuda()
	#print("SUM:", torch.sum(T))
	A = torch.exp(-C/beta).float().cuda()
	#print('Begin IPOT:', 'M:',torch.mean(M).item(), 'C:',torch.mean(C).item(), 'SIGMA:',torch.mean(sigma).item())
	for t in range(iteration):
		Q = A * T # bs * n * m
		for k in range(1):
		...
		# 	  ' A:',torch.mean(A).item(),
		# 	  ' delta:',torch.mean(delta).item(),
		# 	  ' a:',torch.mean(a).item(),
		# 	  ' sigma:',torch.mean(sigma).item(),
		# 	  ' T:',torch.mean(T).item())
		#T = torch.mul(T_, delta * Q * sigma.transpose(2,1))
