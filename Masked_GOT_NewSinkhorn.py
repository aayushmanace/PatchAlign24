...
def cost_matrix_batch_torch(x, y):
	...
	#print(x)
	y = y.div(torch.norm(y, p=2, dim=1, keepdim=True))
	cos_dis = torch.bmm(torch.transpose(x.double(), 1, 2), y.double())#.transpose(1,2)
	cos_dis = 1 - cos_dis # to minimize this value	
	# cos_dis = - cos_dis
	#print(cos_dis.transpose(2,1))
...
def IPOT_distance_torch_batch_uniform(C, M, bs, n, m, iteration=20):
	C = C.float().cuda()
	T = IPOT_torch_batch_uniform(C, M, bs, n, m, iteration=iteration)
    #print(T.shape)
...
# def IPOT_torch_batch_uniform(C, M, bs, n, m, beta=0.5, iteration=50):
# 	# C is the distance matrix
# 	# c: bs by n by m
...
# 	T_ = torch.ones_like(T)
# 	for t in range(iteration): 
# 		Q = A * T # bs * n * m
# 		Q_ = A * T_
# 		for k in range(1):
# 			delta = 1 / (n * torch.bmm(Q_, sigma) + 1e-4)
# 			a = torch.bmm(torch.transpose(Q,1,2), delta)
# 			sigma = 1 / (float(m) * a + 1e-4)
# 		T = delta * Q * sigma.transpose(2,1)
# 		T_ = delta * Q_ * sigma.transpose(2,1)
# 	return T#.detach() 
...
def IPOT_torch_batch_uniform(C, M, bs, n, m, beta=0.5, iteration=50):
	...
	T  = M.cuda()#torch.ones(bs, n, m).cuda()#M.cuda()#M.cuda()#torch.ones(bs, n, m).cuda()#M.cuda()
	A = torch.exp(-C/beta).float().cuda()
	for t in range(iteration):
		Q = A * T # bs * n * m
		for k in range(1):
...
def GW_distance(X, Y, M, p, q, lamda=0.5, iteration=5, OT_iteration=100):
	'''ṭ
	...
	m = Ct.size(2)
	n = Cs.size(2)
	T, Cst = GW_torch_batch(Cs, Ct, M, bs, n, m, p, q, beta=lamda, iteration=iteration, OT_iteration=OT_iteration)
	temp = torch.bmm(torch.transpose(Cst,1,2), T)
	distance = batch_trace(temp, m, bs)
	return distance

def GW_torch_batch(Cs, Ct, M, bs, n, m, p, q, beta=0.5, iteration=5, OT_iteration=20):
	...
	for i in range(iteration):
		...
		# gamma = a * K * b
		gamma = IPOT_torch_batch_uniform(C_gamma, M, bs, n, m, beta=beta, iteration=OT_iteration)
...
def GW_distance_uniform(X, Y, M, lamda=1e-1, iteration=5, OT_iteration=20):
	m = X.size(2)
	n = Y.size(2)
	bs = X.size(0)
	p = (torch.ones(bs, m, 1)/m).cuda()
	q = (torch.ones(bs, n, 1)/n).cuda()
	return GW_distance(X, Y, M , p, q, lamda=lamda, iteration=iteration, OT_iteration=OT_iteration)
