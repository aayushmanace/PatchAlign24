...
class Network(torch.nn.Module):
    ...
    def forward(self, x, attribute=None):
        ...
        if self.choice == 'vit_disco':
            ...
            out_1 = self.branch_1(feature_map)
            out_2 = self.branch_2(feature_map)
            out_4 = self.project_head(torch.mean(feature_map,dim=1))
            feature_map_detach = feature_map.detach()
            out_3 = self.branch_2(feature_map_detach)
            out_5 = self.multilabelclassifier(torch.mean(feature_map,dim=1))
            return [torch.mean(out_1, dim=1), torch.mean(out_2, dim=1), torch.mean(out_3, dim=1),out_4 ,out_5]
            ...
            return [torch.mean(out_1, dim=1), torch.mean(out_2, dim=1), torch.mean(out_3, dim=1), out_mask.view(-1, 197, 256), feature_map]
