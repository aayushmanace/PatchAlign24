...
class BinaryMatrixGenerator(nn.Module):
    ...
    def forward(self, x):
        ...
        return torch.round(x.view(-1, self.a, self.b))
...
class Network(torch.nn.Module):
    ...
    def forward(self, x, attribute=None):
        if self.choice == 'disentangle':
            ...
            return [torch.mean(out_1, dim=1), torch.mean(out_2, dim=1), torch.mean(out_3, dim=1), torch.round(out_mask.view(-1, 197, 256)), feature_map]
            # return [out_1, out_2, out_3]
